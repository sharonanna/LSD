...
def train_fn(loader, model, optimizer, loss_fn, scaler, writer, slope_factor):
    loop = tqdm(loader)
...
def main():
    ...
    for sf in range(0,11):
        print(f"SLOPE FACTOR:{sf}")
        for epoch in range(NUM_EPOCHS):
            print(f"Epoch:{epoch}")
            train_fn(train_loader, model, optimizer, loss_fn, scaler, writer, slope_factor=sf)
            ...
            check_accuracy(val_loader, model, loss_fn, slope_factor=sf, device=DEVICE)

            # save_predictions_as_imgs( 
            #     val_loader, model,slope_factor=sf, device=DEVICE
            # )
