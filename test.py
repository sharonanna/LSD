...
PIN_MEMORY = False
LOAD_MODEL = True
TEST_IMG_DIR = "test_data_size/test_images"
TEST_IMG_MASK = "test_data_size/test_masks"
SLOPE_DIR = "test_data_size/slope_images"
...
def save_predictions_as_imgs(loader, model, slope_factor, device="cuda"):
    ...
    for idx, (x,y,s) in enumerate(loader):
        ...
        with torch.no_grad():
            probs = torch.softmax(model(x),dim=1)
            preds = torch.argmax(probs, dim=1)
            np.save('test_predictions_size/tm_org'+str(idx)+'.npy',x)
            np.save('test_predictions_size/tm_preds'+str(idx)+'.npy',preds)
            np.save('test_predictions_size/tm_y'+str(idx)+'.npy',y)
...
def main():
    ...
    if LOAD_MODEL:
        load_checkpoint(torch.load("checkpoint_exp4_1.pth.tar"), model)
