...
def save_predictions_as_imgs(loader, model, slope_factor, device="cuda"):
    ...
    for idx, (x,y,s) in enumerate(loader):
        ...
        with torch.no_grad():
            preds = torch.sigmoid(model(x))
            preds = (preds > 0.5).float()
            np.save('cd_test_preds_binary/tm_org'+str(idx)+'.npy',x)
            np.save('cd_test_preds_binary/tm_preds'+str(idx)+'.npy',preds)
            np.save('cd_test_preds_binary/tm_y'+str(idx)+'.npy',y)
...
def main():
    ...
    if LOAD_MODEL:
        load_checkpoint(torch.load("checkpoint_exp7_10_binary.pth.tar"), model)
