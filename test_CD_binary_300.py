...
def save_predictions_as_imgs(loader, model, slope_factor, device="cuda"):
    ...
    for idx, (x,y,s) in enumerate(loader):
        ...
        with torch.no_grad():
            preds = torch.sigmoid(model(x))
            preds = (preds > 0.5).float()
            np.save('test_predictions_binary_300_cd/tm_org'+str(idx)+'.npy',x)
            np.save('test_predictions_binary_300_cd/tm_preds'+str(idx)+'.npy',preds)
            np.save('test_predictions_binary_300_cd/tm_y'+str(idx)+'.npy',y)
