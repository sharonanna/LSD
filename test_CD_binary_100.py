...
PIN_MEMORY = False
LOAD_MODEL = True
TEST_IMG_DIR = "test_data_B100/test_images"
TEST_IMG_MASK = "test_data_B100/test_masks"
SLOPE_DIR = "test_data_B100/slope_images"
...
def save_predictions_as_imgs(loader, model, slope_factor, device="cuda"):
    ...
    for idx, (x,y,s) in enumerate(loader):
        ...
        with torch.no_grad():
            preds = torch.sigmoid(model(x))
            preds = (preds > 0.5).float()
            np.save('test_predictions_binary_100_cd/tm_org'+str(idx)+'.npy',x)
            np.save('test_predictions_binary_100_cd/tm_preds'+str(idx)+'.npy',preds)
            np.save('test_predictions_binary_100_cd/tm_y'+str(idx)+'.npy',y)
...
def main():
    ...
    if LOAD_MODEL:
        load_checkpoint(torch.load("checkpoint_exp7_100_5.pth.tar"), model)
